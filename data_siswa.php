...
<style>
    ...
    <title>Pendaftaran Murid Baru SMK PGRI 05 Jember 2021</title>
</head>
<body>
...
<div class="container">
  ...
  <div class="card-body">
    ...
    <tr>
        ...
        <td><?=$data['jurusan'];?></td>
        <td><?=$data['email'];?></td>
        <td><img src="image/<?= $data['foto']; ?>" style="width: 100px;float: left;margin-bottom: 4spx;"></td>
        <td>
            <a href="data.php?hal=edit&id=<?=$data['id_siswa']?>" class="btn btn-info">Edit</a>
            <a href="data.php?hal=hapus&id=<?=$data['id_siswa']?>"
            onclick="return confirm('Apa anda yakin ingin menghapus data ini?')" class="btn btn-danger">Hapus</a>
        </td>
    </tr>
    <?php } ?>
    </table>
